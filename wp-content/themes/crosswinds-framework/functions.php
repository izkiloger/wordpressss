...
function crosswinds_framework_is_admin_page( $page = null ) {
	...
	if ( null === $page && ( ( 'themes.php' === $pagenow ) && ( 'crosswinds-framework-theme' === $_GET['page'] || 'crosswinds-framework-options' === $_GET['page'] ) ) ) {
		return true;
	}
...
function crosswinds_framework_get_suggested_plugins() {
	$plugins = array(
		array(
			'name'     => 'Crosswinds Blocks',
			'slug'     => 'crosswinds-blocks',
			'logo'     => 'https://ps.w.org/crosswinds-blocks/assets/icon-256x256.png',
			'link'     => 'https://wordpress.org/plugins/crosswinds-blocks/',
			'location' => 'internal'
		),
		array(
...
function crosswinds_framework_view_documentation() {
	...
	<div class="options-section">
		...
		<a class="button cf-button-primary" href="<?php echo esc_url( apply_filters( 'crosswinds_framework_documentation_link', 'https://crosswindsframework.com/docs/crosswinds-framework/' ) ); ?>"><?php esc_html_e( 'View Documentation', 'crosswinds-framework' ); ?></a>
	</div>
	<?php
}
add_action( 'crosswinds_framework_admin_documentation_section', 'crosswinds_framework_view_documentation' );

function crosswinds_framework_list_child_themes() {
	if ( ! crosswinds_framework_if_child_theme_active() ) {
		?>
		<div class="options-section">
			<h2><?php esc_html_e( 'Build Your Website Even Quicker with a Crosswinds Framework Child Theme', 'crosswinds-framework' ); ?></h2>
			<p><?php esc_html_e( 'Cut down the time you spend building your website by using one of the Crosswinds Framework child themes! Each of these themes focus on a specific niche to give you a great design to match what you need to build. It\'s a great way to get a head start on creating an amazing website for yourself or your business!', 'crosswinds-framework' ); ?></p>
			<a class="button cf-button-primary" href="https://crosswindsframework.com/themes/"><?php esc_html_e( 'View Child Themes', 'crosswinds-framework' ); ?></a>
		</div>
		<?php
	}
}
add_action( 'crosswinds_framework_theme_settings', 'crosswinds_framework_list_child_themes' );
